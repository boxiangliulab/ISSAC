### ISSAC test data (Docker version — works on Mac, Windows)

# Docker image to use
ISSAC_IMAGE=yuntian1999/issac:v1.3

# Wrap `docker run` as a function so the rest of the script can call
# $ISSAC exactly like a local binary. We mount the current working
# directory into the container at the same path, and set the
# container's working directory to match, so all relative paths
