...
cell_state<-read.table(paste("/home/users/nus/e0950183/scratch/brain_cell_state/meta",cell,"PC",test,"_value.csv",sep=""),sep=",",header=TRUE)
cell_state_PC<-as.data.frame(matrix(NA,1,ncol(PC)))
colnames(cell_state_PC)<-colnames(PC)
for(i in 1:nrow(cell_state)){
    cell_state_PC[1,cell_state$X[i]]<-cell_state$PC[i]
}

sorted_PC<-sort(cell_state$PC)
upper_third_index <- ceiling(length(sorted_PC)*(2/3))
lower_third_index <- ceiling(length(sorted_PC)*(1/3))
upper_third<-sorted_PC[upper_third_index]
lower_third<-sorted_PC[lower_third_index]

final_result<-as.data.frame(matrix(NA,nrow(newdat),13))
...
if(nrow(newdat)<end)end=nrow(newdat)
...
for(i in start:end){
    ...
    state=as.numeric(cell_state_PC[1,]),
    ...
    num<-which(model_data$state<lower_third)
    newmodel<-model_data[num,]
    model <- glmer(cbind(suc, fal) ~ sPC1 + sPC2 + sPC3 + sPC4 + sPC5 + sPC6 + sPC7 + sPC8 + gPC1 + gPC2 + gPC3 + gPC4 + gPC5 + 
        sex + age + ROS_MAP + PMI + num_cells + educ + data_source + geno, 
               data = newmodel, 
               family = binomial(link="logit"))
    ...
    final_result$V12[i]<-computeRegressionSlope(as.numeric(pheno_revise[num]),as.numeric(newmodel$geno))
    ...
    final_result$V13[i]<-computeRegressionSlope(as.numeric(pheno_revise[num]),as.numeric(newmodel$geno))
}

final_result<-final_result[start:end,]
final_result$V7<-p.adjust(final_result$V6,method="fdr",n=nrow(final_result))

write.table(final_result,paste("/home/users/nus/e0950183/scratch/brain_cell_state/result/",cell,"_result_PC",test,"_",seq,sep=""),sep="\t",row.names=FALSE,col.names=FALSE,quote=FALSE)
